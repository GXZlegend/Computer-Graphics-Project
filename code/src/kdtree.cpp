...
float getArea(Vector3f v0, Vector3f v1) {
    Vector3f b = v1 - v0;
    return b[0] * b[1] + b[0] * b[2] + b[1] * b[2];
}

float splitArea(Vector3f v0, Vector3f v1, int splitDim, float splitPlane, bool left) {
...
float findSplitPlane(std::vector<Box> &box, Vector3f v0, Vector3f v1, int splitDim, bool &flag) {
...
void KDTree::buildKDTree(std::vector<Box> &rootBox, int depth, int splitDim) {
...
void KDTree::getBoxes(std::vector<Box> &data) {
    if (!leafbox.empty())
